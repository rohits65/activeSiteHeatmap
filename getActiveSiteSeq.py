...
def getResidueMap(mapArray, targets, scores, dist):
    
    for i in range(0, len(targets)):
        for j in range(1, len(scores[i])+1):
            if scores[i][j-1] != 99:
                mapArray.append(activeSite(str(targets[i]), str(targets[i]) + "0.pdb", "UNK", targets[i] + "3." + str(j) + ".pdb", scores[i][j-1], dist))
...
if __name__ == "__main__":
    ...
    data, cols = getResidueMap(mapArray, targets, scores, 2.75)
